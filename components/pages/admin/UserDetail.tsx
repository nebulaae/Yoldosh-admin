  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Link
        ...
        className="flex items-center gap-2 w-full text-emerald-500 hover:underline"
      >
      ...
      <Card className="component shadow-none">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <InfoItem
            icon={<Wallet />}
            label="Баланс"
            value={`${Number(user.wallet?.balance || 0).toLocaleString(
              "ru-RU",
              { style: "currency", currency: user.wallet?.currency || "UZS" }
            )}`}
          />
          <InfoItem
            icon={<Car />}
          ...
          <InfoItem
            ...
            label="Блокировка до"
            value={
              user.banExpiresAt ? formatDate(user.banExpiresAt) : "Не установлено"
            }
          />
          <InfoItem
            icon={<MessageSquare />}
      ...
      {user.notificationPreferences && (
        <Card className="component shadow-none">
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {Object.entries(user.notificationPreferences).map(
              ([key, value]) => (
                <InfoItem
                  key={key}
                  icon={<Mail />}
      ...
      <Tabs defaultValue="trips">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="trips">
            {user.role === "Driver" ? "Поездки водителя" : "Поездки пассажира"}
          </TabsTrigger>
        ...
        <TabsContent value="trips" className="space-y-4 mt-4">
          {user.role === "Driver"
            ? user.drivenTrips?.length
              ? user.drivenTrips.map((trip: any) => (
                <TripCard key={trip.id} trip={trip} />
              ))
              : <p className="text-muted-foreground p-4 text-center">
                У водителя нет поездок.
              </p>
            : user.bookingsAsPassenger?.length
              ? user.bookingsAsPassenger.map((booking: any) => (
                <BookingCard key={booking.id} booking={booking} />
              ))
              : <p className="text-muted-foreground p-4 text-center">
                У пассажира нет поездок.
              </p>}
        </TabsContent>
