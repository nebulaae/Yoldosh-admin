...
import { useGetUserDetails, useBanUser } from "@/hooks/adminHooks";
...
import { User, Mail, Phone, Car, Wallet, Calendar, ShieldAlert } from "lucide-react";
...
export const UserDetail = ({ userId }: { userId: string }) => {
    ...
    const totalSpent = user.bookingsAsPassenger?.reduce((acc: any, booking: any) => {
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                     {user.avatar ? (
                        <img src={user.avatar} alt={`${user.firstName} ${user.lastName}`} className="w-20 h-20 rounded-full object-cover" />
                    ) : (
                    ...
                    <div>
                        <h1 className="title-text">{user.firstName} {user.lastName}</h1>
                    ...
                    <DialogContent>
                        ...
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(onBanSubmit)} className="space-y-4">
                                ...
                                <Button type="submit" variant="destructive" disabled={isBanning}>{isBanning ? 'Блокировка...' : 'Заблокировать'}</Button>
                            </form>
                        </Form>
                    </DialogContent>
                </Dialog>
            ...
            <Card className="dark:bg-slate-900">
                <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <InfoItem icon={<ShieldAlert />} label="Статус" value={user.isBanned ? "Заблокирован" : "Активен"} valueClassName={user.isBanned ? "text-destructive" : "text-green-500"} />
                    <InfoItem icon={<Phone />} label="Телефон" value={user.phoneNumber} />
                    <InfoItem icon={<Car />} label="Всего поездок" value={user.role === 'Driver' ? user.drivenTrips?.length || 0 : user.bookingsAsPassenger?.length || 0} />
            ...
            <Tabs defaultValue="trips">
                ...
                <TabsContent value="trips" className="space-y-4 mt-4">
                    {user.role === 'Driver' && (user.drivenTrips?.length > 0 ? user.drivenTrips.map((trip: any) => <TripCard key={trip.id} trip={trip} />) : <p className="text-muted-foreground p-4 text-center">У водителя нет поездок.</p>)}
                    {user.role === 'Passenger' && (user.bookingsAsPassenger?.length > 0 ? user.bookingsAsPassenger.map((booking: any) => <BookingCard key={booking.id} booking={booking} />) : <p className="text-muted-foreground p-4 text-center">У пассажира нет поездок.</p>)}
                </TabsContent>
                <TabsContent value="cars" className="space-y-4 mt-4">
                     {user.cars?.length > 0 ? user.cars.map((car: any) => (
                        <CarCard key={car.id} car={car} />
                    )) : <p className="text-muted-foreground p-4 text-center">У пользователя нет машин.</p>}
                </TabsContent>
                <TabsContent value="reports" className="space-y-4 mt-4">
                     {user.receivedReports?.length > 0 ? user.receivedReports.map((report: any) => (
...
const InfoItem = ({ icon, label, value, valueClassName }: { icon: React.ReactNode, label: string, value: string | number, valueClassName?: string }) => (
    <div className="flex items-center gap-4">
    ...
    <Card className="dark:bg-slate-900">
        <CardHeader>
            <div className="flex justify-between items-center">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(trip.status)}`}>{trip.status}</span>
            </div>
            <p className="text-sm text-muted-foreground">{formatDate(trip.departure_ts)}</p>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><p className="text-sm text-muted-foreground">Откуда</p><p>{trip.fromVillage?.nameRu ?? 'N/A'}</p></div>
            <div><p className="text-sm text-muted-foreground">Куда</p><p>{trip.toVillage?.nameRu ?? 'N/A'}</p></div>
             <div><p className="text-sm text-muted-foreground">Стоимость</p><p>{parseFloat(trip.price_per_person).toLocaleString('ru-RU')} UZS</p></div>
        </CardContent>
    </Card>
)
...
const BookingCard = ({ booking }: { booking: any }) => (
     <Card className="dark:bg-slate-900">
        <CardHeader>
         ...
         <CardContent>
            <div><p className="text-sm text-muted-foreground">Стоимость</p><p>{parseFloat(booking.totalPrice).toLocaleString('ru-RU')} UZS</p></div>
        </CardContent>
    </Card>
)
...
const CarCard = ({ car }: { car: any }) => (
     <Card className="dark:bg-slate-900">
        <CardHeader>
             <div className="flex justify-between items-center">
                <CardTitle className="text-base">{car.modelDetails?.make} {car.modelDetails?.model}</CardTitle>
        ...
        <CardContent>
            <p className="text-sm text-muted-foreground">Цвет: {car.color}, Год: {car.carYear}</p>
        </CardContent>
    </Card>
)
...
const ReportCard = ({ report }: { report: any }) => (
     <Card className="dark:bg-slate-900">
        <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">От: {report.reportingUser?.firstName || 'N/A'} - {formatDate(report.createdAt)}</p>
        </CardHeader>
        <CardContent>
